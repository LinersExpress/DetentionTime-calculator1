...
const formatDuration = (ms) => { const totalMinutes = Math.floor(ms / 60000); const hours = Math.floor(totalMinutes / 60); const minutes = totalMinutes % 60; const decimalHours = (ms / 3600000).toFixed(2); return ${hours}h ${minutes}m (${decimalHours}); };

const handleCalculate = () => { const start = new Date(offDutyDateTime); const end = new Date(onDutyDateTime); if (isNaN(start) || isNaN(end) || end <= start || isNaN(parseFloat(hourlyRate))) return;
...
return ( <div style={{ backgroundColor: "#000000", minHeight: "100vh", color: "#ffffff", padding: "1rem", fontFamily: "Arial, sans-serif" }}> <h1 style={{ fontSize: "2rem", fontWeight: "bold" }}>Detention Time Calculator</h1>
